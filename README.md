...
<div align="center"><sub>A recorded session on the chart tab: candles from the spot tape (badge says so), sweep/block/split markers at each event's time and spot, and the three per-print panes, notes verbatim underneath. Seeded synthetic feed.</sub></div>
...
- **Markers: the engine's sweeps, blocks, and splits**, at each event's timestamp and spot, sized by premium (log scale), colored by aggressor side, calls above the bar and puts below, shape by kind. Hover for the print, click for the same event drawer the flow table opens: the full score breakdown, the reasons trail, and the NBBO each leg was judged against. The premium floor applies to markers (they are emitted events) and to nothing else on the chart.
...
- **GEX levels (toggle):** per-strike net GEX from the current chain drawn as bars anchored to the price axis, zero-gamma dashed, with the dealer-convention assumption and the pricing line in a legend that stays on the chart.
